...
Mesh::Mesh(const std::string &filename, btScalar scale)
{
    load(filename, scale);
}

std::vector<Mesh::Item> Mesh::parse(const std::string &filename)
{
...
void Mesh::load(const std::string &filename, btScalar scale)
{
    vertices.clear();
    polygons.clear();
...
btVector3 *Mesh::points()
{
    return vertices.data();
}

std::size_t Mesh::points_count() const
{
    return vertices.size();
}

void Mesh::set_points(btConvexHullShape &shape)
{
